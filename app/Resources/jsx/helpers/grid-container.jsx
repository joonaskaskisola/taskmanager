...
export default class GridContainer extends React.Component {
	...
	sortByColumn(e) {
		...
		this.setState({'sortOrder': this.state.sortBy !== sortBy ? true : !this.state.sortOrder});
	...
	render() {
		...
		this.props.columns.forEach(function (column, i) {
			columns.push(<div name={self.props.fields[i]} onClick={self.sortByColumn} key={'column-' + column}
			                  className='column clickable'>{ column }</div>);
		});
		...
		this.state.results.forEach(function (row) {
			if (self.props.viewRow) {
				rows.push(<Row app={self.props.app} fields={self.props.fields} row={row} key={row.id}
				               viewRow={self.props.viewRow}/>);
			} else {
				rows.push(<Row app={self.props.app} fields={self.props.fields} row={row} key={row.id}
