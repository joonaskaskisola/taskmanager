	...
	render() {
		...
		this.props.fields.forEach(function (field) {
			...
			if (field.indexOf('.') !== -1) {
				...
				fields.push(<div key={'field-' + field + self.props.row.id}
				                 className='column'>{ self.props.row[field] }</div>);
			}
		});

		return <div className='row' onClick={ () => {
			hashHistory.push('/' + this.props.app + '/' + this.props.row.id);
			this.props.viewRow(this.props.row.id)
		}}>
			{fields}
		</div>
	}
